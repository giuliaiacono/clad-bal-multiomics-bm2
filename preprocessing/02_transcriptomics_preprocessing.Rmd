...
samples_sequenced_detailed <- data.frame(SampleID = sequenced) %>% 
  ...
  mutate(Group = ifelse(clad_clinical == TRUE, "CLAD", "CLAD-free"),
         ...
         sample = gsub("-", "", sample),
         sample = ifelse(SampleID == "5134", "CLADfree_22_382", sample), # change duplicate from batch 3 to 22_382 so different patient days
...
rna_seq_metadata <- samples_sequenced_detailed %>%
  filter(!SampleID %in% c("3672", "4138", "5134")) # 3672 is right native, 4138 is sex mismatched, and 5134 is duplicate from batch 3, I kept 5134a from batch 1 instead and this one was uploaded to SRA
