...
lip_sample_metadata <- read_csv("Lipidomics/Data/combined/combined-metadata-lip.csv", col_types = 'ncccc') %>%
  filter(SampleID != "3673") %>% # remove R healthy lung, (left tx one is 3705)
...
write.csv(small_molecules, "small_molecules_table.csv")
write.csv(small_molecules_info, "small_molecules_info_table.csv")
...
write.csv(small_molecules_metadata %>% dplyr::select(!c("ParticipantInitials", "Gender", "Age", "transplant_date", "Donor_gender", "Donor_age")), "small_molecules_metadata_table.csv")
```

# Annotation checks

```{r}
a <- small_molecules_info %>% 
  left_join(rbind(metab_annotated_filtered_info_df[, c("shortname", "info.Adduct type", "info.Post curation result", "info.Average Rt(min)", "info.Average Mz", "ionisation")],
                  lip_annotated_filtered_info_df[, c("shortname", "info.Adduct type", "info.Post curation result", "info.Average Rt(min)", "info.Average Mz", "ionisation")] )
            ) %>%
  relocate(shortname, `info.Adduct type`, `info.Average Rt(min)`, `info.Average Mz`, `info.Post curation result`) %>%
  filter(!`info.Adduct type` %in% c("[M+H]+", "[M-H]-")) %>%
  mutate(DA = ifelse(shortname %in% c(biomarker_molecules_combined, stable_molecules_combined), "DA", NA)) %>%
  filter(!is.na(DA))
write.csv(a, "check-misannotation.csv")
...
adducts <- data.frame(cbind(adduct_name, adduct_mass)) %>% filter(adduct_name %in% a$`info.Adduct type`)

s <- SummarizedExperiment(rowData = a[, c("shortname", "info.Adduct type", "info.Average Mz", "info.Post curation result", "info.Metabolite name")])

corrected <- add_hmdb_corrected(s, hmdb, 0.002, 6) 

b <- data.frame(rowData(corrected))
```
