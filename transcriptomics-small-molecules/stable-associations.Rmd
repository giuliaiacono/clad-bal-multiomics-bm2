...
```{r}
dds <- DESeqDataSetFromMatrix(countData = stable_rna,
                              colData = stable_rna_metadata,
                              design = ~1)  
vsd <- vst(dds, blind=FALSE)

a <- data.frame(adonis2(formula = as.formula("t(assay(vsd)) ~Months_grouped+Transplant_indication+Total_ant_num+GIT_ischemic_time+Record.ID"), 
                    data = colData(vsd), 
                    distance = "eu", 
                    dfun = vegdist, 
                    permutations = 999,
...
ggsave("pca/rna-pcoa-months-grouped.pdf", p, width = 13, height = 7, units = 'cm')
```
...
# *Differential expression months
...
gene_links <- data.frame("Link" = g$V1, 
  ...
  arrange(Gene)
write.csv(gene_links, "deseq/months/gene-links.csv", row.names = FALSE)
...
data <- rna_combined_heat %>%
  ...
  t() %>%
  as.data.frame()
saveRDS(data, "tmixclust/rna/stable/data.rds")
...
saveRDS(cluster_obj, "tmixclust/rna/stable/cluster_obj.rds") 
...
saveRDS(cluster_genes, "tmixclust/rna/stable/cluster_genes.rds")
...
saveRDS(cluster_rna_list, "tmixclust/rna/stable/cluster_rna_list.rds")
```
...
p <- 
  ...
  scale_x_discrete(expand = c(0,0.5))
ggsave("tmixclust/rna/stable/rna-clusters.pdf", p, width = 9, height = 13, units = 'cm')
...
write.csv(gene_cluster_assignment %>% arrange(Gene), "tmixclust/rna/stable/gene_cluster_assignment.csv", row.names = FALSE)
```
...
ggsave("tmixclust/rna/stable/rna-clusters-cont.pdf", p, width = 6, height = 12, units = 'cm')
```
...
```{r}
deseq_list$months$cluster_genes$ego_simp[[3]]@result %>% arrange(p.adjust) %>% filter(p.adjust < 0.05, Count >= 6) # Down up
deseq_list$months$cluster_genes$ego[[2]]@result %>% filter(p.adjust < 0.05, Count >= 6) %>% arrange(p.adjust) %>% filter(p.adjust <= 3.706392e-03 | Description == "nitric oxide metabolic process")  # Down
...
rbind(
  deseq_list$months$cluster_genes$ego_simp[[3]]@result %>% filter(p.adjust < 0.05, Count > 7) %>% arrange(p.adjust) %>%
    mutate(Cluster = "Innate\nimmunity"), # down
  
  deseq_list$months$cluster_genes$ego[[1]]@result %>% filter(p.adjust < 0.05) %>% arrange(p.adjust) %>%
    mutate(Cluster = "Cell\nsignalling"), # up
  
deseq_list$months$cluster_genes$ego_simp[[2]]@result %>% filter(p.adjust < 0.05, Count > 7) %>% arrange(p.adjust) %>% 
  mutate(Cluster = "Adaptive\nimmunity")  # downup
) %>%
  dplyr::select(!qvalue)

s <- strsplit(stable_rna_pathways$geneID, split = "/")
ne <- data.frame("Gene" = unlist(s),
                 "Description" = rep(stable_rna_pathways$Description,sapply(s, length)),
                 "Pathway_id" = rep(stable_rna_pathways$ID,sapply(s, length)), 
                 "Count" = rep(stable_rna_pathways$Count,sapply(s, length)), 
                 "adj_pvalue" = rep(stable_rna_pathways$p.adjust,sapply(s, length)),
                 "log_adjpvalue" = rep(stable_rna_pathways$log_adjpvalue,sapply(s, length)),
                 "Cluster" = rep(stable_rna_pathways$Cluster,sapply(s, length))
                 ) 
...
ggplot(data, aes(x = Value, y = reorder(Description, -log(adj_pvalue)))) +
  geom_point(aes(fill = -log(adj_pvalue), size = Count), shape = 21) +
  theme +
  theme(legend.position = "right", legend.justification = "left", legend.box = "vertical", legend.title = element_text(face = "bold")) +
    theme(axis.text.x = element_blank(),
        axis.line = element_blank(),
...
ggsave("tmixclust/rna/stable/stable-pathways.pdf", p, width = 16, height = 14, units = 'cm')
ggsave("tmixclust/rna/stable/stable-pathways.png", p, width = 16, height = 14, units = 'cm', dpi = 600)
```
...
# Annotation
text_list <- read.csv("tmixclust/rna/stable/stable-rna-heatmap.csv") %>% filter(Heatmap_annotation == TRUE) %>% pull(Gene)
...
text_annotation <- anno_mark(at = row_position %>% pull(Row) %>% as.numeric(), 
                                                   labels = row_position %>% pull(Genes), 
                                                   labels_gp = gpar(fontsize = 12, fontface = "bold"),
                                 which = "row")

text_annotation <- rowAnnotation(text_annotation = text_annotation)
...
png("tmixclust/rna/stable/stable-heat-interval-all.png", width = 18, height = 25, units = "cm", res = 500)
p
invisible(dev.off())
```
