---
title: "Biomarker cohort - Molecules associations post-transplant"
author: "Giulia Iacono"
date: Sys.Date()
output: html_document
---

# Biomarker cohort - Molecules associations post-transplant
...
biomarker_small_molecules_metadata %>% group_by(Group) %>% dplyr::count(Record.ID) %>% group_by(Group) %>% dplyr::count() # 10 vs 13
```
...
ms_classes <- read.csv(paste0(analysis_path, "transcriptomics-small-molecules/limma/group+months/molecules/biomarker-molecules-heatmap.csv"))
biomarker_classes <- molecules_list$`group+months`$GroupCLAD$sig_hits %>% 
  left_join(ms_classes[, c("Molecule", "Class", "Heatmap_annotation", "Rename", "KEGG")]) %>%
  as.data.frame() %>%
  arrange(direction, Class) %>%
  mutate(Class = factor(Class, levels = c(unique(Class)[-grep("Organooxygen compounds and other", unique(Class))] , "Organooxygen compounds and other") ))
...
m <- biomarker_classes %>% arrange(direction, Class)
...
ms_combined_heat_intervals <- ms_combined_heat_intervals[m$Molecule, ] 
...
identical(rownames(ms_combined_heat_intervals), m$Molecule) # TRUE
identical(colnames(ms_combined_heat_intervals), rownames(df)) # TRUE

# Abbreviated molecules
rownames(ms_combined_heat_intervals) <- m$Rename
```
...
png(paste0(analysis_path, "transcriptomics-small-molecules/limma/group+months/molecules/biomarker-heat-interval.png"), width = 14, height = 25, units = "cm", res = 600)
h
invisible(dev.off())
```
...
ggsave(paste0(analysis_path, "transcriptomics-small-molecules/limma/group+months/molecules/alluvium.png"), width = 4, height = 25, units = "cm", dpi = 600)
```
...
compounds <- biomarker_classes %>% filter(direction == a) %>% pull(KEGG) %>% gsub(";.*", "", .) %>% na.omit() %>% as.character()
