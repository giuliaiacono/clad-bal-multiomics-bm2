---
title: "Stable cohort - Microbial factors of variation"
author: "Giulia Iacono"
date: Sys.Date()
output: html_document
---

# Stable cohort - Microbial factors of variation
# Adonis bacteria
...
ggplot(data.frame(sample_data(bact.logcss)), aes(x = Transplant_indication, y = Diversity)) +
  ...
  labs(x = "Transplant indication", 
    ...
    stat_compare_means(label = "p.format", method = "wilcox.test", comparisons = comparisons) + # Add pairwise comparisons p-value
  scale_y_continuous(limits = c(0, 6.5)) +
  plot_fill(Transplant_indication = TRUE) +
  theme(legend.position = "none")

ggsave("bact-tx-shannon.png", p, width = 10, height = 8, units = 'cm')

wilcox_test(data.frame(sample_data(bact.logcss)), Diversity ~ Transplant_indication, ref.group = ".all.") 
pairwise_wilcox_test(data.frame(sample_data(bact.logcss)), Diversity ~ Transplant_indication) 
```
...
ggplot(m, aes(x = Transplant_indication, y = Abundance)) +
  ...
  stat_compare_means(label = "p.format", method = "wilcox.test", comparisons = comparisons, tip.length = 0.01) +
  scale_y_continuous(limits = c(-1, 27)) +
  plot_fill(Transplant_indication = TRUE)

ggsave("tx-pseudomonas.png", p, width = 10, height = 8, units = 'cm')
```
