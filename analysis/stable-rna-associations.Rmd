...
ggsave(paste0(analysis_path, "stable-rna-associations/pca/rna-pcoa-months-grouped.pdf"), p, width = 13, height = 7, units = 'cm')
```
...
# Limma Differential expression months

```{r}
design <- model.matrix(~ns(Months, df = 3) + Transplant_indication + Total_ant_num + GIT_ischemic_time, stable_rna_metadata)
dupcor <- duplicateCorrelation(stable_rna, design = design, block = stable_rna_metadata$Record.ID)
vfit <- lmFit(stable_rna, design, block = stable_rna_metadata$Record.ID, correlation = dupcor$consensus)
efit <- eBayes(vfit)
