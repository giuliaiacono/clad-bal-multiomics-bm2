...
```{r}
b <- read.csv(paste0(preprocessing_path, 'Microbiome/Data/seq-metadata.csv')) %>%
  filter(Record.ID %in% c(stable_patients, biomarker_patients))

b %>%
  filter(Sequenced == TRUE, Sample_excluded == FALSE, Type == "Sample", Days < 800) %>%
  filter(grepl("16S.", SampleID) )
```
