...
ggsave(paste0(analysis_path, "cmv-bal.png"), p, width = 12, height = 8, units = 'cm')
```
...
cmv_blood <- master_metadata$cmv_blood %>% 
  dplyr::select(Record.ID, Days, IfPositiveCMVBALViralLoad) %>%
  filter(Record.ID %in% biomarker_patients) %>% 
  ...
  mutate(Group = factor(Group, levels = c("CLAD-free", "CLAD")),
         ...
         Record.ID = factor(Record.ID)) %>%
  filter(is.na(days_to_clad) | days_to_clad >= -30, Months < 26) %>%
  mutate(cmv_result = case_when(IfPositiveCMVBALViralLoad > 0 ~ "Positive",
                                is.na(IfPositiveCMVBALViralLoad) ~ "Negative"))
...
d <- master_metadata$bronch_metadata %>% 
  #filter(Patient_days %in% all_selected_samples_collected_across_datasets) %>%
  filter(Record.ID %in% unique(c(stable_patients, biomarker_patients))) %>%
...
p <-  
  ggplot(d, aes(x = Months, y = Record.ID)) +
...
ggsave(paste0(analysis_path, "clinical-metadata/azithromycin.png"), p, width = 12, height = 12, units = 'cm', dpi = 600)
```
