---
title: "Stable cohort - Transcriptomics associations post-transplant"
author: "Giulia Iacono"
date: Sys.Date()
output: html_document
---

# Stable cohort - Transcriptomics associations post-transplant
# Transcriptomics
...
data <- as.data.frame(assay(deseq_list$months$vsd))
data <- data[, colData(deseq_list$months$vsd)$Patient_days]
identical(colnames(data), rownames(colData(deseq_list$months$vsd))) #TRUE

a <- data.frame(adonis2(formula = t(data)~Days_interval+Transplant_indication+Total_ant_num+GIT_ischemic_time+Record.ID, 
                    data = colData(deseq_list$months$vsd), 
                    distance = "eu", 
                    dfun = vegdist, 
                    permutations = 999,
                    na.action = na.omit,
...
ggsave(paste0(analysis_path, "transcriptomics-small-molecules/pca/rna-pcoa-months-grouped.png"), p, width = 13, height = 7, units = 'cm', dpi = 600)
```
...
# checked if values range between -1 and 1
