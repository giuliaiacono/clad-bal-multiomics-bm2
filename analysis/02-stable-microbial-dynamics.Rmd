...
model_full <- lmer(Diversity ~ ns(Months, 3) + Transplant_indication + Total_ant_num + GIT_ischemic_time + (1 | Record.ID),
  data = d,
  REML = FALSE
)

model_reduced <- lmer(Diversity ~ Transplant_indication + Total_ant_num + GIT_ischemic_time + (1 | Record.ID),
  data = d,
  REML = FALSE
)
...
emm <- emmeans(model_full, ~ Months, at = list(Months = c(1, 6)) )
confint(contrast(emm, method = "revpairwise"), level = 0.95)

#Bacterial Shannon Diversity increased by 0.53 between 1 and 6 months post-transplant (95% CI 0.304–0.757, p = 0.0005081)
...
#Fungal Shannon Diversity increased by 0.45 between 1 and 6 months post-transplant (95% CI 0.175–0.721, p = 0.001386)
