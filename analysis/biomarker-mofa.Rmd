...
p <- ggplot(r2t, aes(fill=Var1, y=value, x=Var1)) +
  scale_fill_manual(values=c("#56CFE1", "#FF6348FF", "blue")) + 
...
ggsave(paste0(analysis_path, "biomarker-mofa/total-var.pdf"), p, width = 6, height = 6, units = 'cm')
...
ggsave(paste0(analysis_path, "biomarker-mofa/var-per-factor.pdf"), p, width = 12, height = 6, units = 'cm')
```
...
```{r}
  ...
  ggsave(paste0(analysis_path, "biomarker-mofa/factor2-months-lm.pdf"), p, width = 10, height = 7, units = 'cm')
```
...
ggplot(aes(x = value, y = reorder(feature, abs(value)))) +
  ...
  labs(title = paste("Factor 1 features"),
       y = "Feature",
       x = "Weight value") +
  facet_grid(view~., scales = "free", space = "free") +
  scale_fill_manual(values = c("Transcripts" = "#56CFE1", "Proteins" = "#FF6348", "Bacteria" = "blue"))

ggsave(paste0(analysis_path, "biomarker-mofa/weights/factor1.pdf"), p, width = 8, height = 15, units = 'cm')
```

```{r}
mofa_rna_features <- fact_weights_list[[1]] %>% filter(view == "Transcripts") %>% slice_max(abs(value), n = 50) %>% pull(Feature) %>% as.character()
mofa_molecules_features <- fact_weights_list[[1]] %>% filter(view == "Molecules") %>% slice_max(abs(value), n = 50) %>% pull(Feature) %>% as.character()

intersect(mofa_rna_features, biomarker_rna_combined_df$Gene)
intersect(mofa_molecules_features, biomarker_molecules_classes$Molecule)
```
